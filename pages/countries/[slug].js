...
const data = {
  ...
  'france': { title:'Study in France', banner:'/banners/france.png', content:(
    <>
      ...
      <ul className="list-disc pl-6 space-y-2 mt-4 text-gray-700">
        <li><strong>World-Class Education and Global Recognition</strong> - France hosts prestigious universities and grandes ecoles; degrees are valued worldwide.</li>
    ...
    <>
      ...
      <section className="bg-gradient-to-b from-white via-[#f7f9ff] to-[#eef2ff] py-12 lg:py-16">
        <div className="container grid lg:grid-cols-3 gap-8 items-start">
          ...
          >
            <div className="overflow-hidden rounded-3xl border border-white/70 bg-white/80 backdrop-blur shadow-[0_24px_90px_-45px_rgba(15,23,42,0.55)]">
              <img src={item.banner} alt={item.title} className="w-full h-64 object-cover" />
            </div>
            <CountryContent>
              {typeof item.content === 'string' ? (
                <p className="text-gray-700 leading-relaxed">{item.content}</p>
              ) : (
                <div className="text-gray-700 leading-relaxed space-y-4">{item.content}</div>
              )}
            </CountryContent>
          </motion.div>
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <p className="text-sm font-semibold text-gray-800">Country specialists ready to help</p>
            </div>
            <p className="text-sm text-gray-700 leading-relaxed">Get a tailored shortlist and visa prep checklist for {item.title}. We share realistic budgets, timelines, and scholarship options.</p>
            <div className="space-y-2 text-sm">
              <a href="/contact" className="block rounded-xl bg-brand-blue text-white px-4 py-2.5 font-semibold text-center shadow-lg hover:-translate-y-0.5 transition-transform">Talk to an expert</a>
              <a href="tel:+919391572075" className="block rounded-xl border border-brand-blue/40 text-brand-blue px-4 py-2.5 font-semibold text-center hover:bg-brand-blue/5 transition-colors">+91 93915 72075</a>
              <a href="mailto:info@mysmartscholar.com" className="block rounded-xl border border-gray-200 text-gray-800 px-4 py-2.5 font-semibold text-center hover:bg-gray-50 transition-colors">info@mysmartscholar.com</a>
            </div>
            <div className="rounded-2xl border border-gray-100 bg-gray-50 px-4 py-3 text-sm text-gray-700">
              <p className="font-semibold text-gray-900">What you receive</p>
              <ul className="list-disc pl-5 mt-2 space-y-1">
                <li>Admission and scholarship guidance</li>
                <li>Visa mock interviews and file checks</li>
                <li>Pre-departure and housing pointers</li>
              </ul>
            </div>
          </motion.div>
        </div>
